...
def ema_update(model_dest: nn.Module, model_src: nn.Module, rate: float):
    param_dict_src = dict(model_src.named_parameters())
    for p_name, p_dest in model_dest.named_parameters():
        if p_name not in param_dict_src:
            continue
        p_src = param_dict_src[p_name]
        if p_dest.data.device != p_src.data.device:
            p_src_data = p_src.data.to(p_dest.data.device)
        else:
            p_src_data = p_src.data
        p_dest.data.mul_(rate).add_((1 - rate) * p_src_data)


def load_pretrained_generic(model, ckpt_path, logger=None):
    ckpt = torch.load(ckpt_path, map_location="cpu")
    state = ckpt.get("state_dict", ckpt)
    missing, unexpected = model.load_state_dict(state, strict=False)

    if logger is not None:
        logger.warning(f"[Pretrained load] Missing keys count: {len(missing)}")
        logger.warning(f"[Pretrained load] Unexpected keys count: {len(unexpected)}")
        if len(missing) > 0:
...
def get_selected_ratio(model_unwrapped, device):
    selected_ratio_list = []
    for block in model_unwrapped.blocks:
        mlp = getattr(block, "mlp", None)
        ratio = getattr(mlp, "last_selected_ratio", None)
        if ratio is not None:
            selected_ratio_list.append(ratio)

    if len(selected_ratio_list) > 0:
        return torch.stack(selected_ratio_list).mean()
    ...
    for epoch in range(start_epoch + 1, config.num_epochs + 1):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if load_vae_feat:
                ...
                with torch.no_grad():
                    with torch.cuda.amp.autocast(enabled=config.mixed_precision == "fp16"):
                        ...
                        if getattr(config, "sample_posterior", False):
                            z = posterior.sample()
                        else:
                            z = posterior.mode()
            ...
            with accelerator.accumulate(model):
                ...
                router_loss = getattr(model_unwrapped, "router_loss", None)
                if router_loss is None:
                    router_loss = torch.tensor(0.0, device=clean_images.device)

                loss = diffusion_loss + router_loss
            ...
            model_unwrapped = accelerator.unwrap_model(model)
            selected_ratio = get_selected_ratio(model_unwrapped, clean_images.device)
            ...
            logs = {
                ...
                "selected_ratio": selected_ratio.item(),
            }
...
def parse_args():
    parser = argparse.ArgumentParser(description="Selective FFN training")
    ...
    parser.add_argument(
        ...
        type=str,
        default="tensorboard",
        help='Supported: "tensorboard", "wandb", "comet_ml", or "all".',
    )
    parser.add_argument(
        "--tracker_project_name",
        type=str,
        default="text2image-fine-tune",
        help="Project name for Accelerator trackers",
    )
...
if __name__ == "__main__":
    ...
    if config.load_from is not None:
        missing, unexpected = load_pretrained_generic(model, config.load_from, logger=logger)
        logger.warning(f"[Pretrained load] missing={len(missing)}, unexpected={len(unexpected)}")
    ...
    selective_trainable_count = 0
    for block in model.blocks:
        if getattr(block, "use_selective_ffn", False):
            for p in block.mlp.parameters():
                p.requires_grad = True
                selective_trainable_count += p.numel()

    logger.info(f"Trainable selective params: {selective_trainable_count}")
    ...
    logger.info(f"EMA DEVICE: {next(model_ema.parameters()).device}")

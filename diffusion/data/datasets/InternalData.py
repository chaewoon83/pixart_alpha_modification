...
class InternalData(Dataset):
    def __init__(self,
        ...
        for json_file in image_list_json:
            ...
            self.img_samples.extend([os.path.join(self.root, 'images', item['path']) for item in meta_data_clean])
            self.txt_feat_samples.extend([os.path.join(self.root, 'caption_feature_wmask', '_'.join(item['path'].rsplit('/', 1)).replace('.jpg', '.npz')) for item in meta_data_clean])
            self.vae_feat_samples.extend([os.path.join(self.root, f'img_vae_features_{resolution}resolution/noflip', '_'.join(item['path'].rsplit('/', 1)).replace('.jpg', '.npy')) for item in meta_data_clean])
